# 📊 TutorNow Analytics Project

## 🧠 Overview
This project simulates a real-world tutoring platform called **TutorNow**, based in Sydney, Australia. The goal is to design and implement a complete data pipeline to analyze student demand, tutor supply, pricing trends, and engagement behavior.

The project demonstrates an end-to-end workflow from raw data generation and cleaning to data warehousing and business intelligence visualization.

---

## 🎯 Objectives
- Analyze student distribution across Sydney suburbs  
- Evaluate tutor supply vs demand by subject and location  
- Identify pricing trends based on experience and subject  
- Understand booking patterns and seasonal trends  
- Measure student retention and tutor performance  

---

## 🏗️ Architecture

CSV Dataset → Python (Cleaning & Feature Engineering) → Snowflake (Data Warehouse) → Power BI (Dashboard)

---

## 🛠️ Tech Stack

- **Python (Pandas)** → Data cleaning & feature engineering  
- **Snowflake** → Cloud data warehouse & SQL transformations  
- **Power BI** → Interactive dashboards & visualization  
- **SQL** → Data modeling (fact & dimension tables)  

---

## 📊 Dashboard (Power BI)

The Power BI dashboard provides an interactive view of tutoring market dynamics across Sydney, enabling insights into demand, pricing, and student engagement patterns.

### 📌 Overview
- Total sessions, revenue, completion rate  
- Demand and activity breakdown across key suburbs  
- Market performance KPIs  

### 💰 Pricing Insights
- Hourly rate vs tutor experience  
- Subject-wise pricing distribution  
- Premium pricing patterns across high-demand subjects  

### 📈 Demand Trends
- Session volume over time (monthly trends)  
- Peak booking days and seasonal demand patterns  

### 🔁 Retention & Performance
- Repeat student rate analysis  
- Tutor rating vs student retention relationship  

---

## 📸 Dashboard Preview

### 🔗 Live Dashboard
👉 [View Power BI Dashboard](https://app.powerbi.com/view?r=eyJrIjoiMmQ0OTc3M2QtMmM1Zi00NzlhLWIwNTItYWU5MTM1ZTQ2NjU2IiwidCI6ImJlODMyOWE3LTcyMTgtNDlhMy05YWMxLWQ3Yjk1NDU2M2YzOSIsImMiOjEwfQ%3D%3D)

---

### 🖼️ Screenshots

#### 📊 Overview Dashboard
<img width="1445" height="830" alt="image" src="https://github.com/user-attachments/assets/5e9cf476-3fb2-41b5-9db9-532e6df56033" />

#### 📊 Pricing Insights Dashboard
<img width="1448" height="823" alt="image" src="https://github.com/user-attachments/assets/3b69cd57-24c9-43d9-9283-8b749fdaf4ce" />

---

## 🔍 Key Insights

- High student demand observed in suburbs like Parramatta and Sydney CBD  
- STEM subjects (especially Coding) command higher hourly rates  
- Tutors with higher ratings tend to retain more students  
- Peak demand occurs during weekdays and exam periods  
- Discounts have moderate impact on booking volume  

---

## 🚀 Future Improvements
- Add incremental data pipeline (daily updates)  
- Integrate dbt for transformation management  
- Build machine learning model for demand prediction  
- Deploy dashboard to Power BI Service  

---

## 🧠 Skills Demonstrated
- Data Cleaning & Feature Engineering (Python)  
- SQL & Data Modeling (Snowflake)  
- Data Warehousing Architecture  
- Business Intelligence & Dashboarding (Power BI)  
- Analytical Thinking & Insight Generation  

---

## 📬 Contact
Feel free to connect or reach out for feedback or collaboration.
